...
export const EnhancedExperienceSection = () => {
  ...
  const typeDetails: { [key: string]: { icon: React.ReactNode; color: string } } = {
    certification: { icon: <Award className="h-5 w-5" />, color: "text-blue-400" },
    education: { icon: <GraduationCap className="h-5 w-5" />, color: "text-green-400" },
    hackathon: { icon: <Trophy className="h-5 w-5" />, color: "text-yellow-400" },
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4">
        <div className="text-center mb-20">
          ...
          <p className="text-lg md:text-xl text-gray-400 max-w-3xl mx-auto">
            A timeline of key certifications, educational milestones, and competitive achievements.
          </p>
        </div>
            ...
            {(selectedSkill ? experiences.filter(e => e.skills.some(s => s.toLowerCase().includes(selectedSkill.toLowerCase()))) : experiences).map((exp, index) => {
              ...
              const imageSide = (
                <div className="w-full md:w-5/12 hidden md:flex items-center justify-center">
                  <div className="w-full p-4 group" style={{ perspective: "1000px" }}>
                    <img
                      src={exp.image}
                      alt={`${exp.title} Image`}
                    ...
                    <div className="relative z-10">
                      <div className="flex items-start gap-4">
                        <div className="flex-shrink-0 w-16 h-16 rounded-lg bg-gray-800/50 flex items-center justify-center border border-gray-700/50 group-hover:scale-105 group-hover:border-teal-500/50 transition-all duration-300">
                          <IconComponent className={`w-8 h-8 ${detail.color}`} />
                        </div>
                        <div className="flex-grow">
                          <p className={`text-sm font-bold uppercase tracking-wider mb-1 ${detail.color}`}>
                            {exp.organization}
                          </p>
                          <h3 className="text-xl font-bold text-white mb-2">{exp.title}</h3>
                          <div className="flex items-center gap-4 text-gray-400 text-sm">
                            <span className="flex items-center gap-1.5">{detail.icon} <span className="capitalize">{exp.type}</span></span>
                            <span className="flex items-center gap-1.5"><Calendar className="h-4 w-4" /> {exp.period}</span>
                          </div>
                        </div>
                      </div>
                      <p className="text-gray-300 leading-relaxed text-base my-4">{exp.description}</p>
                      <div className="flex flex-wrap gap-2">
                        {exp.skills.map((skill) => <Badge key={skill}>{skill}</Badge>)}
                      </div>
                      <div className={`transition-all duration-500 ease-in-out overflow-hidden ${isActive ? "max-h-96 opacity-100 mt-4" : "max-h-0 opacity-0"}`}>
                       ...
                       <div className="text-center mt-4 text-xs text-gray-500 group-hover:text-teal-400 transition-colors">
                            ...
                            <ChevronRight className={`inline-block h-3 w-3 ml-1 transition-transform ${isActive ? "rotate-90" : ""}`} />
                        </div>
                    </div>
                  </div>
                </div>
              ...
              return (
                <div key={exp.id} className={`relative flex items-center ${isEven ? 'md:flex-row-reverse' : 'md:flex-row'}`}>
                  <div className="absolute left-4 md:left-1/2 -translate-x-1/2 w-8 h-8 bg-gray-800 rounded-full border-2 border-gray-700 flex items-center justify-center z-10">
                    <div className="w-5 h-5 bg-black rounded-full flex items-center justify-center">
                        <div className={`p-0.5 ${detail.color} animate-pulse`}>{React.cloneElement(detail.icon as React.ReactElement, { className: "w-3 h-3" })}</div>
                    </div>
                  </div>
                  {contentSide}
                  {imageSide}
